...
fetchISIdleMetric()
{
    ...
    echo "*******************************************" >> /home/hadoop/EMRShutdown/log.sh
    echo "$ENDTIME" >> /home/hadoop/EMRShutdown/log.sh
    echo "IsIdle metric returned from CloudWatch : $EMRISIDLE" >> /home/hadoop/EMRShutdown/log.sh
...
if [ $ISEMRUSED -eq 1 ]; then
    emruse=1
    fetchISIdleMetric
    echo "isIdle:$isIdle" >> /home/hadoop/EMRShutdown/log.sh
    isActiveSSh
    echo "isAnyActiveSSH:$ISSSH" >> /home/hadoop/EMRShutdown/log.sh
    yarnJobsStatusChk
    echo "Active-Spark-Jobs:$ISACTIVEYARN" >> /home/hadoop/EMRShutdown/log.sh
	...
	if [[ ( $ISSSH -eq 0 ) && ( $NonZRunning -eq 0 ) && ( $presto_InProgress -eq 0 ) && ( $ZTASKACTIVE -eq 0) ]]; then
        ...
        echo "Other-Running:$NonZRunning" >> /home/hadoop/EMRShutdown/log.sh
        echo "PrestoInprogress:$presto_InProgress" >> /home/hadoop/EMRShutdown/log.sh
        echo "Zeppelin-Running:$ZRunning" >> /home/hadoop/EMRShutdown/log.sh
        echo "Zeppelin-ActiveTask:$ZTASKACTIVE" >> /home/hadoop/EMRShutdown/log.sh
        echo "isEMRInUse:$emruse"
        echo "isEMRInUse:$emruse" >> /home/hadoop/EMRShutdown/log.sh
        echo "  " >> /home/hadoop/EMRShutdown/log.sh
    fi
    ...
    if [ $VERIFY -eq 0 ]; then
        ...
        aws cloudwatch put-metric-data --metric-name "isEMRUsed-"$jobflowid --value "$emruse" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "Active-Yarn-Jobs-"$jobflowid --value "$ISACTIVEYARN" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "sshMetric-"$jobflowid --value "$ISSSH" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "idleMetric-"$jobflowid --value "$isIdle" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "NonZRunning-"$jobflowid --value "$NonZRunning" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "PrestoActive-"$jobflowid --value "$presto_InProgress" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "ZRunning-"$jobflowid --value "$ZRunning" --unit "Count" $CLOUDWATCH_OPTS
        aws cloudwatch put-metric-data --metric-name "zActive-"$jobflowid --value "$ZTASKACTIVE" --unit "Count" $CLOUDWATCH_OPTS
        aws s3 cp /var/lib/ganglia/rrds/ "s3://aws-logs-$accountid-$region/ganglia/$jobflowid/" --recursive

        if [ $ALARM_CONFIGURED -eq 0 ]; then
            aws cloudwatch put-metric-alarm --alarm-name "shutDown-"$jobflowid --alarm-description "Shut down unused "$jobflowid --metric-name "isEMRUsed-"$jobflowid --namespace EMRShutdown/Cluster-Metric --statistic Average --period 300 --threshold 0 --comparison-operator LessThanOrEqualToThreshold  --dimensions  Name=JobFlowId,Value=$jobflowid --evaluation-periods 9 --unit "Count" --alarm-actions arn:aws:sns:eu-west-1:$accountid:terminate-emr
